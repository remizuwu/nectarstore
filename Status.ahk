...
nm_command(command)
{
	...
	switch (name := params[1]), 0
	{
		...
		if WinExist("natro_macro.ahk ahk_class AutoHotkey") {
			...
			if messageOut!=1
				return discord.sendEmbed("failed to store nectar because: " (messageOut = 0 ? "Roblox not found!" : messageOut = 2 ? "Unable to detect yOffset!" : messageOut = 3 ? "Nectar " params[2] " is empty!" : "unhandled error!" messageOut), 0xFF0000 + 0), command_buffer.removeAt(1)
		}
		DetectHiddenWindows 1
		if WinExist("natro_macro.ahk ahk_class AutoHotkey") {
			detectHiddenWindows 0
